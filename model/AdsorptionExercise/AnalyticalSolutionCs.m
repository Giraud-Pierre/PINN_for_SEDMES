...
Cgplot = forward(net,cat(1,xdlplot,tdlplot));
[CgExact, CsExact] = solveAdsorptionPointByPoint(xplot,tplot);

err = norm(extractdata(Cgplot) - CgExact) / norm(CgExact)

figure()
plot3(xplot,tplot,extractdata(Cgplot),'*','DisplayName',"Cs predicted")
hold on
plot3(xplot,tplot,CgExact,'*', 'DisplayName', "exact Cs")
legend()